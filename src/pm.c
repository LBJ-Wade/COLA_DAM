...
{
  // Assume FFTW3 is initialized in lpt.c //DAM sure??
  int nc_pm_factor=(int)(nc_pm/pow((double)np_total,1./3.)+0.5);

  Ngrid=nc_pm; NgridL= nc_pm;
  BoxSize=boxsize;
  PM_factor=nc_pm_factor;

  msg_printf(info,"Estimated PM factor %d\n",nc_pm_factor);
  ...
  const int ncp=(int)(pow((double)np_total,1./3.));
  NParticleTotal=np_total;
...
static void PtoMesh(const Particle P[],const int np,float* const density)
{
  // ** precondition
...
static void compute_density_k(void)
{
...
static void compute_force_mesh(const int axes)
{
...
static void force_at_particle_locations(const Particle P[],const int np, 
					const int axes, 
...
static int send_buffer_positions(Particles* const particles,BufferVec3* buf)
{
...
static void add_buffer_forces(Particles* const particles,BufferVec3* buf)
{
  const int np_local=particles->np_local;
